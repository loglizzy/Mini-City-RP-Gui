...
local Window = library:AddWindow("gay rp", {
...
do -- viadagens
    do -- anti down
        ...
        player.CharacterAdded:Connect(function()
            update()
            if character:FindFirstChild('Downed') and enabled and checkscript() then
                character.Downed.Value = false
                character.Downed.RobloxLocked = true
                character.Downed:Remove()
            end
        end)
        
        Misc:AddSwitch('Anti down (atordoado)', function(v)
            enabled = v
            if not v then
                update()
                if not character:FindFirstChild('Downed') then
                    Instance.new('BoolValue', character).Name = 'Downed'
                end
            end
        end)
    end
    ...
    do -- anti slow
        ...
        local function disable()
            update()
            if character:FindFirstChild('AntiSpeed') and enabled and checkscript() then
                character.AntiSpeed.Disabled = true
            end
        end
        
        player.CharacterAdded:Connect(disable)
        
        Misc:AddSwitch('Anti slow when down', function(v)
            enabled = v
            if not v then
                character.AntiSpeed.Disabled = false
            else
                disable()
            end
        end)
    ...
    do -- god mode
        local enabled
        local remote = game.ReplicatedStorage.Revive
        ...
        Misc:AddSwitch('Auto Revive (basicly god mode)', function(v)
            enabled = v
        end)
    end
